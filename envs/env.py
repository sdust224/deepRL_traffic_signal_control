...
class PhaseMap:
    ...
    def get_phase_num(self, phase_id):
        return self.phases[phase_id].num_phase  # for large grid scenario num_phase is 5
...
class TrafficSimulator:
    def __init__(self, config, output_path, is_record, record_stats, port=0):
        ...
        self.control_interval_sec = config.getint('control_interval_sec')  # 5s
        self.yellow_interval_sec = config.getint('yellow_interval_sec')  # 2s
        self.episode_length_sec = config.getint('episode_length_sec')  # 3600s
        self.T = np.ceil(self.episode_length_sec / self.control_interval_sec)  # 720
        ...
        for node_name in self.node_names:
            ...
            if self.agent == 'greedy':
                ...
                for nnode_name in node.neighbor:
                    if self.agent != 'ma2c' and self.agent != 'mqld':
                        cur_state.append(self.nodes[nnode_name].wave_state)
                    else:
                        # discount the neighboring states
                        cur_state.append(self.nodes[nnode_name].wave_state * self.coop_gamma)  # coop_gamma = 0.9
                # include wait state
                if 'wait' in self.state_names:
                    cur_state.append(node.wait_state)
                # include fingerprints of neighbors
                if self.agent == 'ma2c' or self.agent == 'mqld':
                    for nnode_name in node.neighbor:
    ...
    def _init_nodes(self):
        ...
        for node_name in self.sim.trafficlight.getIDList():
            ...
            lanes_out = self.sim.trafficlight.getControlledLinks(node_name)
            ...
            ilds_out = []
            for link_name in lanes_out:
                for lane_name in link_name:
                    for lane in lane_name:
                        ild_name = lane
                        if ild_name not in ilds_out and ild_name not in ilds_in and ild_name[0] != ':':
                            ilds_out.append(ild_name)
            nodes[node_name].ilds_out = ilds_out
    ...
    def _init_action_space(self):
        ...
        for node_name in self.node_names:
            node = self.nodes[node_name]
            phase_id = self._get_node_phase_id(node_name)  # for large grid phase_id is 5
            node.phase_id = phase_id
            node.n_a = self.phase_map.get_phase_num(phase_id)
            self.n_a_ls.append(node.n_a)
        # for global coop level
        self.n_a = np.prod(np.array(self.n_a_ls))  # global action
    ...
    def _init_state_space(self):
        # consider incorporate pressure into state feature
        self._reset_state()
        self.n_s_ls = []
        self.n_w_ls = []
        self.n_f_ls = []
        for node_name in self.node_names:
            ...
            for nnode_name in node.neighbor:
                ...
                if self.agent == 'ma2c' or self.agent == 'mqld':
                    # only ma2c and mqld use neighbor's policy
    ...
    def _measure_reward_step(self):
        ...
        for node_name in self.node_names:
            ...
            for ild in self.nodes[node_name].ilds_in:
                if self.obj in ['queue', 'pressure', 'hybrid']:
                    if self.name == 'real_net':
                ...
                if self.obj in ['wait', 'pressure', 'hybrid']:
                    max_pos = 0
                    car_wait = 0
                    if self.name == 'real_net':
            ...
            for ild in self.nodes[node_name].ilds_out:
                if self.obj in ['pressure']:
                    if self.name == 'real_net':
                        cur_exit_queue = min(10, self.sim.lane.getLastStepHaltingNumber(ild))
                    else:
                        if ild.split('_')[1][1] == 'p':
                            continue
                        else:
                            cur_exit_queue = self.sim.lanearea.getLastStepHaltingNumber(ild)
                    exit_queues.append(cur_exit_queue)

            queue = np.sum(np.array(queues)) if len(queues) else 0
            wait = np.sum(np.array(waits)) if len(waits) else 0
            exit_queue = np.sum(np.array(exit_queues)) if len(exit_queues) else 0
            pressure = np.abs(queue - exit_queue)

            if self.obj == 'queue':
                reward = - queue
            elif self.obj == 'wait':
                reward = - wait
            elif self.obj == 'pressure':
                reward = - pressure - self.coef_wait * wait
            else:
        ...
        for node_name, a in zip(self.node_names, action):
            ...
            for i in range(len(node.waits)):
                ...
                if lane in red_lanes:
                    node.waits[i] += self.control_interval_sec  # 5s
                else:
                    node.waits[i] = 0
    ...
    def step(self, action):
        ...
        if self.agent in ['a2c', 'greedy']:
            reward = global_reward
        elif self.agent != 'ma2c' and self.agent != 'mqld':
            ...
            # discounted global reward for ma2c and mqld
            new_reward = []
            for node_name, r in zip(self.node_names, reward):
                ...
                if self.name != 'real_net':
                    ...
                    n_node = 1 + len(self.nodes[node_name].neighbor)  # local region nodes
