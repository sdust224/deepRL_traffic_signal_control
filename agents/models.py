...
class A2C:
    ...
    def _init_policy(self, n_s, n_a, n_w, n_f, model_config, agent_name=None):
        ...
        if self.name == 'ma2c':  # ia2c or ma2c
            n_fp = model_config.getint('num_fp')  # neighbor layer num 64
...
class IQL(A2C):
    ...
    def _init_policy(self, n_s, n_a, n_w, model_config, agent_name=None):
        if self.model_type == 'dqn':
            n_h = model_config.getint('num_h')  # 64
            n_fc = model_config.getint('num_fc')  # 128
    ...
    def backward(self, summary_writer=None, global_step=None):
        cur_lr = self.lr_scheduler.get(self.n_step)
        # cur_beta = self.beta_scheduler.get_next(self.n_step)
    ...
    def forward(self, obs, mode='act', stochastic=False):
        if mode == 'explore':  # act or explore
            eps = self.eps_scheduler.get(1)  # one step
        action = []
        qs_ls = []  # q_values to policy
        for i in range(self.n_agent):
            qs = self.policy_ls[i].forward(self.sess, obs[i])
            qs = 1 / (1 + np.exp(-qs))  # sigmoid function
...
class MQL(IQL):
    def __init__(self, n_s_ls, n_a_ls, n_w_ls, n_f_ls, total_step,
                 model_config, seed=0, model_type='dqn'):
        self.name = 'mql'
        self.model_type = model_type
        self.agents = []
        self.n_agent = len(n_s_ls)
    ...
    def _init_policy(self, n_s, n_a, n_w, n_f, model_config, agent_name=None):
        ...
        if self.model_type == 'dqn':
            ...
            policy = FPDeepQPolicy(n_s - n_f - n_w, n_a, n_w, n_f, self.n_step, n_fc_wave=n_fw, n_fc_wait=n_ft,
                                   n_fc_fp=n_fp, n_fc0=n_fc, n_fc=n_h, name=agent_name)
        else:
            policy = LRQPolicy(n_s, n_a, self.n_step, name=agent_name)  # FPLRQPolicy()
        return policy
