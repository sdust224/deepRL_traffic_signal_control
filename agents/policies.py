...
class LstmACPolicy(ACPolicy):
    def __init__(self, n_s, n_a, n_w, n_step, n_fc_wave=128, n_fc_wait=32, n_lstm=64, name=None):
        ...
        self.n_w = n_w  # number of wait, 0 or non 0
    ...
    def _build_net(self, in_type, out_type):
        if in_type == 'forward':  # forward or backward
            ob = self.ob_fw
            done = self.done_fw
        else:
            ob = self.ob_bw
...
class QPolicy:
    ...
    def prepare_loss(self, max_grad_norm, gamma):
        self.A = tf.placeholder(tf.int32, [self.n_step])
        self.S1 = tf.placeholder(tf.float32, [self.n_step, self.n_s + self.n_w + self.n_f])
        self.R = tf.placeholder(tf.float32, [self.n_step])
        ...
        with tf.variable_scope(self.name + '_q', reuse=True):
            q1s = self._build_net(self.S1)  # target network
            q1 = tf.reduce_max(q1s, axis=1)
        tq = tf.stop_gradient(tf.where(self.DONE, self.R, self.R + gamma * q1))  # yt
        self.loss = tf.reduce_mean(tf.square(q0 - tq))  # square delta t
...
class DeepQPolicy(QPolicy):
    ...
    def _build_net(self, S):
        if self.n_w == 0:
            h = fc(S, 'q_fcw', self.n_fc0)  # wave 128
        else:
            h0 = fc(S[:, :self.n_s], 'q_fcw', self.n_fc0)  # wave 128
            h1 = fc(S[:, self.n_s:], 'q_fct', self.n_fc0 // 4)  # wait 32
            h = tf.concat([h0, h1], 1)
...
class FPDeepQPolicy(DeepQPolicy):
    def __init__(self, n_s, n_a, n_w, n_f, n_step, n_fc_wave=128, n_fc_wait=32,
                 n_fc_fp=32, n_fc0=128, n_fc=64, name=None):
        QPolicy.__init__(self, n_a, n_s, n_step, 'fpdqn', name)
        self.n_w = n_w
        self.n_f = n_f
        self.n_fc_wave = n_fc_wave
        self.n_fc_wait = n_fc_wait
        ...
        self.n_fc = n_fc
        self.n_fc0 = n_fc0
        self.S = tf.placeholder(tf.float32, [None, n_s + n_w + n_f])
        with tf.variable_scope(self.name + '_q'):
            self.qvalues = self._build_net(self.S)

    def _build_net(self, S):
        h0 = fc(S[:, :self.n_s], 'q_fcw', self.n_fc_wave)
        h1 = fc(S[:, (self.n_s + self.n_w):], 'q_fcf', self.n_fc_fp)
        if self.n_w == 0:
            h = tf.concat([h0, h1], 1)
        else:
            h2 = fc(S[:, self.n_s: (self.n_s + self.n_w)], 'q_fct', self.n_fc_wait)
            h = tf.concat([h0, h1, h2], 1)
        return self._build_fc_net(h, [self.n_fc])