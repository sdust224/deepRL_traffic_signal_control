    ...
    def perform(self, test_ind, demo=False, policy_type='default'):
        ...
        while True:
            if self.agent == 'greedy':
                ...
                # policy-based on-policy learning
                ...
                if policy_type != 'stochastic':
                    action, policy = self.model.forward(ob)
                    if self.agent == 'mqld':
                        self.env.update_fingerprint(policy)
                else:
                    action, policy = self.model.forward(ob, stochastic=True)
                    if self.agent == 'mqld':
                        self.env.update_fingerprint(policy)
            next_ob, reward, done, global_reward = self.env.step(action)
            rewards.append(global_reward)
...
class Tester(Trainer):
    ...
    def run_offline(self):
        # enable traffic measurements for offline test
        is_record = True
        record_stats = False
