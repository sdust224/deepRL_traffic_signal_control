...
from agents.models import A2C, IA2C, MA2C, IQL, MQL
...
def parse_args():
    default_base_dir = '/home/hadoop/Downloads/deeprl_signal_control-master/large_grid'
    default_config_dir = '/home/hadoop/Downloads/deeprl_signal_control-master/config/config_greedy_large.ini'
...
def train(args):
    ...
    if env.agent == 'ia2c':
        ...
        model = MQL(env.n_s_ls, env.n_a_ls, env.n_w_ls, env.n_f_ls, total_step, config['MODEL_CONFIG'],
                    seed=seed, model_type='dqn')
    elif env.agent == 'iqld':
...
def evaluate_fn(agent_dir, output_dir, seeds, port, demo, policy_type):
    ...
    if agent != 'greedy':
        ...
        if agent == 'a2c':
            ...
            model = MQL(env.n_s_ls, env.n_a_ls, env.n_w_ls, env.n_f_ls, 0, config['MODEL_CONFIG'],
                        seed=0, model_type='dqn')
        elif agent == 'iqld':
